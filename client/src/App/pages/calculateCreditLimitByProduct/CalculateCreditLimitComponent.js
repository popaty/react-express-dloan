...
class CalculateCreditLimitByProduct extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rq_body: {
                product_name : "",
                salak_account_number : 0
            },
            loading: false
        };
    ...
    postList = (request) => {
        ...
        fetch('/api/calculateCreditLimitByProduct', {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                if (data.rs_body) {
                    utility.clearSessionStorage("response_calculateCreditLimitByProduct");
                    sessionStorage.setItem("response_calculateCreditLimitByProduct", JSON.stringify(data));
                    window.open('/cclpummary', '_self');
                } else {
    ...
    render() {
        ...
        return (
            <div>
                <DynamicHeader />
                <Container>
                    <h2>Form Input Calculate Credit Limit and Review Date by Product</h2>
                    <Row>
...
export default CalculateCreditLimitByProduct;