...
class TimelineComponent extends Component {
    ...
    Clicked(event) {
        ...
        if (this.state.cif_number == undefined || this.state.year == undefined) {
            alert("Field 'Customer number' and 'year' are required.")
            return
        }
        ...
        setTimeout(() => {
            ...
            var yearmonth = this.state.year;
            console.log(yearmonth);
            if(yearmonth!="" && yearmonth!=undefined){
                yearmonth+= "-"
                if(this.state.month != "" && this.state.month != undefined){
                    yearmonth+=this.state.month;
                }
            } else {
                yearmonth = "";
            }
            fetch('/api/dlake/timeline/' + this.state.cif_number+"/"+yearmonth)
            .then(response => response.json())
            .then(data => {
                if (data) {
                    utility.clearSessionStorage("response_inqTimeline");
                    sessionStorage.setItem("response_inqTimeline", JSON.stringify(data.rs_body));
                    window.open('/timelineSummary', '_self');
                } else {
